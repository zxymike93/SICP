...
(define (execute proc args)
  (cond ((primitive-procedure? proc)
        ...
        (else
         (error "Unknown procedure type -- EXECUTE" proc))))

;;; 以下 analyze* 中先对表达式取分析结果，作为局部变量包含在返回值中，
;;; 而不是将 text-of-quotation 等过程在 lambda 内执行。这样才使得
;;; exp 最简，而不是之后返回的 proc 执行过程中由原生 Scheme 化简。
