#lang racket
;; 不需要明确定义类型之间的强制过程, int->rat
;; 可以定义一个过程，它说明一个类型如何转换到它的父类型
;; 下面的解答并不能处理任意数量的参数
...
(define (level type)
  (let ([t (tag type)])
    (cond [(eq? t 'int) 0]
          [(eq? t 'rat) 1]
          [(eq? t 'com) 2]
          [else (error "LEVEL: " type)])))

(define (raise-all args)
  ;; 找到 types 所有元素中在 level 里最高的元素对应的值
  (define (highest r types)
    (cond [(null? types) (level r)]
          [else
           (cond [(>= (level r) (level (car types)))
                  (highest r (cdr types))]
                 [else
                  (highest (car types) (cdr types))])]))
  ;; raise 一个类型塔中的层次
  (define (raise-one datum count)
    (if (= count 0)
        datum
        (raise-one (raise datum) (- count 1))))
  ;; 给定 highest, args，将 args raise 到最高的类型
  (define (raise-iter h args)
    (cond [(null? args)
           '()]
          [else
           (let ([x (car args)])
             (cons (raise-one x (- h (level x)))
                   (raise-iter h (cdr args))))]))

    (let ([n (highest (car args) args)])
      (raise-iter n args)))
  ...
  (let ([tags (map tag args)])
    (let ([proc (get op tags)])
      (cond [proc
            ...
            [else
             (let ([raised (raise-all args)])
               (let ([tags (map tag raised)])
                 (let ([proc (get op tags)])
                   (cond [proc
                          (apply proc (map content raised))]
                         [else
